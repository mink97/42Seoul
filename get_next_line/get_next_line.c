/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   get_next_line.c                                    :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: mingkang <mingkang@student.42seoul.kr>     +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/21 09:02:48 by mingkang          #+#    #+#             */
/*   Updated: 2022/11/25 19:44:54 by mingkang         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <limits.h>
#include "get_next_line.h"

static ssize_t	read_buf(t_lst *lst, int fd);

static ssize_t	ft_check(t_lst *lst, char *str, ssize_t str_len, int fd)
{
	ssize_t	i;
	ssize_t	check;

	i = 0;
	while (i < str_len && str[i++] != '\n')
		;
	lst->sum += i;
	check = ft_addlst(lst, str, str_len);
	if (check == -1)
		return (-1);
	if (str[i - 1] != '\n')
	{
		check = read_buf(lst, fd);
		if (check == -1)
			return (-1);
	}
	return (i);
}

static ssize_t	read_buf(t_lst *lst, int fd)
{
	char	*buf;
	size_t	i;
	ssize_t	read_len;

	buf = malloc(sizeof(char) * (BUFFER_SIZE));
	if (buf == NULL)
		return (-1);
	i = 0;
	while (i < BUFFER_SIZE)
		buf[i++] = 0;
	read_len = read(fd, buf, BUFFER_SIZE);
	if (read_len > 0)
		read_len = ft_check(lst, buf, read_len, fd);
	free(buf);
	return (read_len);
}

static char	*ft_getstr(t_lst *lst, char **fd_str)
{
	t_lnode	*node;
	char	*str;
	ssize_t	i;
	ssize_t	j;

	str = malloc(sizeof(char) * (lst->sum + 1));
	if (str == NULL)
		return (NULL);
	i = 0;
	node = (lst->head_node.next);
	while (1)
	{
		j = 0;
		while ((node->str)[j] != 0 && i < lst->sum)
			str[i++] = (node->str)[j++];
		if (node->next == 0)
			break ;
		node = (node->next);
	}
	if (i == lst->sum && node->str[j] != 0)
		*fd_str = ft_strldup(&(node->str)[j], ft_strlen(&(node->str)[j]));
	str[i] = 0;
	return (str);
}

char	*get_next_line(int fd)
{
	static char	*fd_arr[OPEN_MAX + 3];
	char		*ret;
	t_lst		*lst;
	ssize_t		len;

	if (BUFFER_SIZE <= 0 || fd < 0 || fd >= OPEN_MAX + 3)
		return (0);
	lst = ft_init_lst(fd_arr + fd);
	if (lst == 0)
		return (0);
	if (fd_arr[fd] != NULL)
	{
		len = ft_check(lst, fd_arr[fd], ft_strlen(fd_arr[fd]), fd);
		free(fd_arr[fd]);
		fd_arr[fd] = NULL;
	}
	else
		len = read_buf(lst, fd);
	ret = 0;
	if (len > 0)
		ret = ft_getstr(lst, &fd_arr[fd]);
	ft_lstclear(lst);
	free(lst);
	return (ret);
}
