...
/*   Updated: 2022/12/12 13:29:00 by mingkang         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

static int	get_min_idx(t_deque *deq)
{
	t_node	*tmp;
	int		idx;
	int		min_value;
...
static void	sort_3(t_deque *deq)
{
...
static void	sort_4(t_deque *deq_a, t_deque *deq_b)
{
	int		min_idx;
...
static void	sort_5(t_deque *deq_a, t_deque *deq_b)
{
	int		min_idx;
...
{
	if (deq_a->cnt == 2 && deq_a->head->data > deq_a->tail->data)
		swap(deq_a, sa);
	else if (deq_a->cnt == 3)
		sort_3(deq_a);
	else if (deq_a->cnt == 4)
